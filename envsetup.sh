...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^aospx_") ; then
       AOSPX_BUILD=$(echo -n $1 | sed -e 's/^aospx_//g')
    else
       AOSPX_BUILD=
    fi
    export AOSPX_BUILD
...
function print_lunch_menu()
{
    ...
    echo
    if [ "z${AOSPX_DEVICES_ONLY}" != "z" ]; then
...
{
    target=$1
    AOSPX_DEVICES_ONLY="true"
    unset LUNCH_MENU_CHOICES
    add_lunch_combo full-eng
    for f in `/bin/ls vendor/aospx/vendorsetup.sh 2> /dev/null`
        do
            echo "including $f"
            . $f
        done
